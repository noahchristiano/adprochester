...
class PasswordResetModel
{
	 ...
	 */
	public static function requestPasswordReset($user_email)
	{
		if (empty($user_email)) {
		...
		$result = UserModel::getUserDataByEmail($user_email);
		if (!$result) {
		...
		// ... and send a mail to the user, containing a link with then token hash string
		$mail_sent = self::sendPasswordResetMail($result->user_email, $user_password_reset_hash);
		if ($mail_sent) {
			return true;
		}
	 ...
	 * @return bool success status
	 */
	public static function sendPasswordResetMail($user_email, $user_password_reset_hash)
	{
		// create email body
	...
	public static function verifyPasswordReset($user_email, $verification_code)
	{
		...
		// check if user-provided email + verification code combination exists
